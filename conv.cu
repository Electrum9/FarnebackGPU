...
there are as many threads as there are output elements in the very final output
(after the vertical pass) assumption is that the image is already padded BEFORE
passing into the convolution filter
*/
__global__ void convolution1DKernel(float *input, float *output,
                                    int input_height, int input_width,
                                    int filter_size, int stride) {
  __shared__ float
      input_tile[TILE_ROWS * stride + 4]
                [TILE_COLS * stride +
                 4]; // to hold the input tile elements (+4 for the halo region)
                     // --> formula comes from reversing the conv output formula
  __shared__ float horizontal_tile[TILE_ROWS * stride + 4]
...
#define bdy blockDim.y // how many rows --> 32 (height)
                    ...
                    int outx =
      bx * blockDim.x + tx;        // x-coordinate of the output
...
/**
 * @brief Generates Gaussian pyramid.
 *
 * @param output Buffer for holding the levels of the Gaussian pyramid. This is
 * expected to initially hold the input image, and should be large enough to
 * contain the entire pyramid.
 * @param input_height  Height of input image.
 * @param input_width   Width of input image.
 * @param num_levels  Number of levels in Gaussian pyramid.
 */
__global__ void pyrDown(float *output, int input_height, int input_width,
                        int num_levels) {
  int total_pixels = input_height * input_width;
  ...
  */
  ...
  for (int i = 1; i < num_levels; i++) {
    convolution1DKernel(output + prev_offset, output + offset, curr_height,
                        curr_width, filter_size, stride);

    curr_height = input_height >> i;
    curr_width = input_width >> i;

    prev_offset = offset;
    offset += total_pixels >> (2 * i);
  }
}

int test_pyrDown(unsigned char* input) {
}

int test_conv() {

  float input[7][7] = {{0, 0, 0, 0, 0, 0, 0}, {0, 1, 2, 3, 2, 1, 0},
                       {0, 4, 5, 6, 5, 4, 0}, {0, 7, 8, 9, 8, 7, 0},
                       {0, 4, 5, 6, 5, 4, 0}, {0, 1, 2, 3, 2, 1, 0},
                       {0, 0, 0, 0, 0, 0, 0}};
  ...
  cudaMemcpy(d_input, input, sizeof(float) * input_height * input_width,
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_filter, filter, sizeof(float) * filter_size,
             cudaMemcpyHostToDevice);
  ...
  convolution1DKernel<<<num_blocks, threads_per_block>>>(
      d_input, d_filter, d_output, input_height, input_width, filter_size,
      stride);

  // Copy the result back to host memory
  cudaMemcpy(output, d_output, sizeof(float) * input_height * input_width,
