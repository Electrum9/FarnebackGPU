...
extern "C"
void process_frame_s2(float* input, float* output, int height, int width) {
    int total = height * width;
    int total_nopad = ((height - 4)/2) * ((width - 4)/2);
    float* d_in;
    float* d_out;
    printf("    height=%d\n", height);
    printf("    width=%d\n", width);
    ...
    // Use the convolution kernel instead of invert_colors
    int filter_size = 5; // Your filter is 5x5
    convolution1DKernel<2,2><<<blocks, threads>>>(d_in, nullptr, nullptr, d_out, height, width, filter_size);
    cudaDeviceSynchronize();
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("CUDA kernel error: %s\n", cudaGetErrorString(err));
